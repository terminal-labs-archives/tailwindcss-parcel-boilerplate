$(document).ready(function () {
  const getResizeableElement = () => {
    return document.getElementById("text-input-pane");
  };
  ...
  const getPaneHeight = () => {
    const pxHeight = getComputedStyle(getResizeableElement()).getPropertyValue(
      "--resizeable-height"
    );
  ...
  const startDragging = event => {
    ...
    const mouseDragHandler = event => {
      ...
      setPaneHeight(yOffset - event.pageY + startingPaneHeight);
      ...
      if (!primaryButtonPressed) {
        setPaneHeight(
          Math.min(Math.max(getPaneHeight(), minPaneSize), maxPaneSize)
        );
        document.body.removeEventListener("pointermove", mouseDragHandler);
        return;
      }
